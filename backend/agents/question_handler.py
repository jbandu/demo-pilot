...
class QuestionHandler:
    ...
    async def handle_question(
        self, question: str, demo_context: Dict[str, Any], current_step: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Generated response - Action: {result['action']}, Sentiment: {result['sentiment']}"
            )
...
4. Works with existing airline systems""",
    ...
    def _build_prompt(
        ...
        if customer_info.get("interests"):
            customer_context += (
                f"Expressed interests: {', '.join(customer_info['interests'])}\n"
            )
...
REMEMBER: You're having a conversation, not reading a script. Be natural!"""
    ...
    def _extract_available_features(
        self, demo_script: List[Dict[str, Any]]
    ) -> List[str]:
    ...
    async def classify_question_batch(
    ...
    async def test_question_handler():
        ...
        for question in test_questions:
            ...
            result = await handler.handle_question(
                question=question, demo_context=demo_context, current_step=3
            )
            ...
            print(
                f"Intent: {result['intent']} | Sentiment: {result['sentiment']} | Priority: {result['priority']}"
            )
