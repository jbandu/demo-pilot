...
@pytest.mark.asyncio
async def test_handle_question_success(question_handler, demo_context, mock_anthropic_response):
    """Test successful question handling"""
    question_handler.anthropic.messages.create = Mock(
        return_value=Mock(content=[Mock(text=mock_anthropic_response['content'][0]['text'])])
    )
    ...
    assert result['action'] in ['continue', 'jump_to_feature', 'deep_dive', 'schedule_human']
...
async def test_handle_question_with_feature_jump(question_handler, demo_context):
    ...
    question_handler.anthropic.messages.create = Mock(
        return_value=Mock(content=[Mock(text='{"answer": "Let me show you!", "action": "jump_to_feature", "feature": "Sign Document", "intent": "feature_request", "sentiment": "positive", "priority": "normal"}')])
    )
