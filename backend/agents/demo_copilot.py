...
class DemoCopilot:
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"handle_question": "handle_question", "next_step": "next_step"},
        )
    ...
    async def _run_workflow(self):
        ...
        try:
            ...
            logger.info(
                f"Completed {result['current_step']}/{len(result['demo_script'])} steps"
            )
    ...
    async def _execute_step(self, state: DemoCopilotState) -> DemoCopilotState:
        ...
        logger.info(
            f"Executing step {current_step + 1}/{len(state['demo_script'])}: {step_data['name']}"
        )
    ...
    async def _handle_question(self, state: DemoCopilotState) -> DemoCopilotState:
        ...
        response = await self.question_handler.handle_question(
            question=question, demo_context=state, current_step=state["current_step"]
        )
    ...
    def _find_step_for_feature(
        self, feature: str, state: DemoCopilotState
    ) -> Optional[int]:
